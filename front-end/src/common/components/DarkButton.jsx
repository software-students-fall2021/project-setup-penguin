 ...
 * @returns
 */
function DarkButton({ btnText, onClick, linkTo, icon, disabled, dark }) {
  const btn = (
    <button
      className={`DarkButton ${disabled && "DarkButton--disabled"}`}
      onClick={onClick}
      disabled={disabled}
    >
      {btnText}
      {icon && <span className="DarkButton__iconContainer">{icon}</span>}
    </button>
  );
